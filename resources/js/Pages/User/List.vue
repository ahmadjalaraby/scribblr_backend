    ...
    <div>
        ...
        <span v-if="props.column.field == 'image'" class="flex items-center">
          ...
          {{ useI18n().t(getGenderFromValue(props.row.gender.value.toLowerCase())) }}
                ...
                <span
                    v-if="props.column.field == 'date_of_birth'"
                    class="text-slate-500 dark:text-slate-400"
                >
          {{ calculateTime(props.row.date_of_birth) }}
        </span>
          ...
          {{ calculateTime(props.row.created_at) }}
                ...
                <span
                    v-if="props.column.field == 'country_id'"
                    class="text-slate-600 dark:text-slate-400"
                >
          {{ props.row.country?.name ?? useI18n().t('dashboard.not_found') }}
        </span>
...
const current = computed(
    () => props.model.users.current_page,
);

function changePage(page) {
    if (page > pageRange.value) return;
    router.get(props.model.users.path + `?page=${page}`)
}
...
const columns = [
    ...
    {
        label: t('dashboard.date_of_birth'),
        field: "date_of_birth",
    },
    {
