<template>
    ...
    <div>
        ...
        perPage: perpage,
            ...
            :rows="props.model.countries.data"
            :select-options="{
        enabled: true,
        ...
        <span v-if="props.column.field == 'image'" class="flex items-center">
          <span class="w-12 h-10 ltr:mr-3 rtl:ml-3 flex-none">
            <img
                :alt="props.row.title"
                :src="useCountryImageUrl(props.row.code).imageUrl"
...
const pageTitle = computed(() => t('dashboard.countries_index'));
...
const current = computed(
    () => props.model.countries.current_page,
);

function changePage(page) {
    if (page > pageRange.value) return;
    router.get(props.model.countries.path + `?page=${page}`)
}
